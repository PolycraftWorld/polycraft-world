...
public class ClientProxy extends CommonProxy
{
	...
	private class PlayerState
	{
		...
		private boolean pogoInAir = false;
	...
	private void setPlayerVelocityFromInputXZ(final EntityPlayer player, final float velocity)
	{
		...
		if (back) directionRadians += -Math.PI;
		if (left) directionRadians += Math.PI / (forward ? -4 : back ? 4 : -2);
		if (right) directionRadians += Math.PI / (forward ? 4 : back ? -4 : 2);
	...
	private void setPlayerVelocityFromInputY(final EntityPlayer player, final float velocity, final boolean cancelGravity)
	{
		if (isKeyDown(gameSettings.keyBindJump))
		{
			...
			player.motionY = -velocity;
		else if (cancelGravity && player.motionY < 0) player.motionY = 0;
	}
	...
	{
		if (isPlayerEntity(event.entity)) playerStates.remove(event.entity);
	}
		...
		if (isPlayerEntity(event.entity))
		{
			...
			else if (isEntityOnBouncyBlock(player))
			{
				if (getBlockUnderEntity(player) instanceof BlockBouncy)
				{
					...
					// if we are supposed to return momentum while not actively jumping (or
					// sneaking)
				...
				} else if (getBlockUnderEntity(player) instanceof BlockBed)
				{
					...
					// if we are actively jumping
					if (noScreenOverlay() && isKeyDown(gameSettings.keyBindJump)) playerState.bouncyBlockBounceHeight = 3; // hard coded bed value spring height
				}
	...
	private void onPlayerTickClientFlashlight(final EntityPlayer player, final PlayerState playerState)
	{
		...
		if (equippedFlashlightRange > 0) ItemFlashlight.createLightCone(player, playerState.flashlightLightSources, equippedFlashlightRange);
	...
	private void onRenderTickItemStatusOverlays(final EntityPlayer player, final PlayerState playerState)
	{
		if (noScreenOverlay())
		{
			...
			if (playerState.airQualityTicksRemaining > 0) playerState.airQualityTicksRemaining--;
							...
							for (int testZ = -8; testZ <= 8; testZ++)
							{
								...
								if (!player.worldObj.isAirBlock(blockX, blockY, blockZ))
								{
									...
									if (testBlock instanceof BlockOre)
									{
										...
										else playerState.cheatInfoOreBlocksFound.put(ore, found + 1);
									}
								}
							}
						}
	...
	private void onClientTickJetPack(final EntityPlayer player, final PlayerState playerState)
	{
		...
		if (ItemJetPack.allowsFlying(player))
		{
			...
			if (keyBindingToggleArmor.isPressed()) jetPackIsFlying = !jetPackIsFlying;
		}
		...
		} else if (playerState.jetPackIsFlying)
		{
			...
			// cause an unstable motion to simulate the unpredictability of the exhaust
			// direction
	...
	private void onPlayerTickClientJetPack(final EntityPlayer player, final PlayerState playerState)
	{
		...
		if (jetPackIgnited) ItemJetPack.createExhaust(player, client.theWorld, playerState.jetPackLightSources, random);
	...
	private void onPlayerTickClientPhaseShifter(final EntityPlayer player, final PlayerState playerState)
	{
		...
		if (isPhaseShifterGlowing) ItemPhaseShifter.createGlow(player, client.theWorld, playerState.phaseShifterLightSources);
	...
	private void onClientTickParachute(final EntityPlayer player, final PlayerState playerState)
	{
		if (ItemParachute.allowsSlowFall(player))
		{
			...
			if (player.motionY < velocityDescent) player.motionY = velocityDescent;
		}
	}

	private void onClientTickRunningShoes(final EntityPlayer player)
	{
		if (ItemRunningShoes.allowsRunning(player) && noScreenOverlay()) if (player.onGround) setPlayerVelocityFromInputXZ(player, ItemRunningShoes.getEquippedItem(player).velocityOnGround);
	}

	private void onClientTickScubaFins(final EntityPlayer player)
	{
		if (ItemScubaFins.allowsFastSwimming(player) && noScreenOverlay())
		{
			if (player.isInWater())
			{
	...
	private void onClientTickGenericMoldedItem(final EntityPlayer player)
	{
		if (ItemMoldedItem.isEquipped(player))
		{
			...
			{
				if (player.isInWater() && !isKeyDown(gameSettings.keyBindJump) && !isKeyDown(gameSettings.keyBindSneak)) player.motionY = 0;
			}
		}
	}
	...
	private void onClientTickPogoStick(final EntityPlayer player, final PlayerState playerState)
	{
		...
		if (ItemPogoStick.isEquipped(player))
		{
			...
			if (!pogoStick.config.restrictJumpToGround || player.onGround || !playerState.pogoInAir)
			{
				final boolean playerActivelyBouncing = isKeyDown(gameSettings.keyBindJump) && noScreenOverlay() && !isPlayerLookingAtPogoCancellingBlock();
				final boolean playerActivelySupressing = !playerActivelyBouncing && isKeyDown(gameSettings.keyBindSneak) && noScreenOverlay();
				final double motionY = pogoStick.config.getMotionY(playerActivelySupressing ? 0 : playerState.pogoStickLastFallDistance, playerState.pogoStickPreviousContinuousActiveBounces,
						playerActivelyBouncing);
				if (motionY > 0)
				{
					player.motionY = motionY;
					playerState.pogoInAir = true;
				}
				if (playerActivelyBouncing)
					playerState.pogoStickPreviousContinuousActiveBounces++;
				else playerState.pogoStickPreviousContinuousActiveBounces = 0;
			}
		} else
		{
		...
		if (player.jumpMovementFactor != jumpMovementFactor) player.jumpMovementFactor = jumpMovementFactor;
	}
	...
	private boolean isPlayerLookingAtPogoCancellingBlock()
	{
		if (client.objectMouseOver != null)
		{
			...
			return block instanceof BlockContainer || block instanceof BlockWorkbench || block instanceof BlockDoor || block instanceof BlockButton || block == Blocks.bed;
		}
		return false;
	}
	...
	private void onClientTickBouncyBlock(final EntityPlayer player, final PlayerState playerState)
	{
		...
		{
			// if the player is on the ground and holding down jump, then wait for the jump
			// to occur
			// (if we try to set the y velocity before the game jumps, it will override our
			// velocity)
			if (!(player.onGround && isKeyDown(gameSettings.keyBindJump)))
			{
				...
				if (motionY > .2) player.motionY = motionY;
	...
	private void onClientTickPhaseShifter(final EntityPlayer player, final PlayerState playerState)
	{
		...
		if (phaseShifterEnabled)
		{
			...
			if (player.isInWater()) player.setAir(baseFullAir);
		}
	}
	...
	public void registerRenderers()
	{
		...
		for (final Inventory inventory : Inventory.registry.values())
		{
			...
			if (inventory.render3D) inventory.renderID = RenderingRegistry.getNextAvailableRenderId();
			...
			else RenderingRegistry.registerBlockHandler(inventory.renderID, renderingHandler = new PolycraftInventoryBlock.BasicRenderingHandler(inventory));

			// TODO: figure out why this is commented out and if it is needed 8.5.2015
			if (inventory.render3D) ClientRegistry.bindTileEntitySpecialRenderer(inventoryBlock.tileEntityClass, renderingHandler);
