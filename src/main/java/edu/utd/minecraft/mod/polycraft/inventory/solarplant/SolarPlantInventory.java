...
public class SolarPlantInventory extends PolycraftInventory {
	...
	public static final void register(final Inventory config) {
		SolarPlantInventory.config = config;
		config.containerType = PolycraftContainerType.SOLAR_PLANT;
		PolycraftInventory.register(new SolarPlantBlock(config, SolarPlantInventory.class));
	}
	...
	public SolarPlantInventory() {
		super(PolycraftContainerType.SOLAR_PLANT, config);
	...
	public static boolean harvestItemsNow(SolarPlantInventory spi) {
	...
	private static boolean doesItemFitInNextSlot(SolarPlantInventory spi, IInventory iinventory, int p_145892_2_, int p_145892_3_) {
	...
	public static IInventory getAssociatedIInventory(SolarPlantInventory spi) {
	...
	private boolean transferInventories() {
		...
		if (!(iinventory instanceof SolarPlantInventory))
			return false;
		...
		else
		{
			for (int i = 0; i < this.getSizeInventory(); ++i)
			{
				if (this.getStackInSlot(i) != null)
				{
					...
					ItemStack itemstack1 = testSlotIndiciesToSpawnItems(((SolarPlantInventory) iinventory), this.decrStackSize(i, 1), Facing.oppositeSide[getDirectionFromMetadata(this.getBlockMetadata())]);
	...
	public static ItemStack testSlotIndiciesToSpawnItems(SolarPlantInventory spi, ItemStack itemStack, int flag) {
	...
	private static ItemStack spawnItemIntoSpecificSlot(SolarPlantInventory iinventory, ItemStack itemStack, int slotIndex, int decWaterAmt) {
		...
		if (canSpawnIntoSlot(iinventory, itemStack, slotIndex, decWaterAmt))
		{
			...
			if (flag)
			{
				if (iinventory instanceof SolarPlantInventory)
				{
					((SolarPlantInventory) iinventory).setTransferCooldown(8 * decWaterAmt);
					iinventory.markDirty();
				}
	...
	private static boolean sunShiningInputSlotsAreWaterAndDecrementThem(SolarPlantInventory spi, int decWaterAmt) {
