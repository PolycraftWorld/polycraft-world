...
public class ItemPolycraftDoor extends Item implements PolycraftItem
{
	public Block doorBlock;
    private Material doorMaterial;
    private static final String __OBFID = "CL_00000020";

    public ItemPolycraftDoor(Material p_i45334_1_, Block block)
    {
    ...
    public boolean onItemUse(ItemStack par1ItemStack,
                             EntityPlayer par2EntityPlayer, World par3World, BlockPos blockPos, EnumFacing par7, float par8, float par9, float par10) {
    	if (par7 != EnumFacing.UP) {
    		...
    		if (par2EntityPlayer.canPlayerEdit(blockPos, par7,
    				par1ItemStack)
    				&& par2EntityPlayer.canPlayerEdit(blockPos.up(),
    						par7, par1ItemStack)) {
    			if (!block.canPlaceBlockAt(par3World, blockPos)) {
    				return false;
    			} else {
    				int i1 = MathHelper
    						.floor_double((double) ((par2EntityPlayer.rotationYaw + 180.0F) * 4.0F / 360.0F) - 0.5D) & 3;
    				placeDoorBlock(par3World, blockPos.getX(), blockPos.getY(), blockPos.getZ(), i1,
    						block);
...
//    	itemIcon = iconRegister.registerIcon(PolycraftMod.getAssetName(PolycraftMod.getFileSafeName("password_door")));
//    }
