...
public class PumpGui extends PolycraftInventoryGui<PumpInventory> {
	...
	protected void drawGuiContainerForegroundLayer(int p_146979_1_, int p_146979_2_) {
		...
		int offset = 30;
		if (flowNetwork.pumpShutOffValve == true) {
			this.fontRendererObj.drawString("Pump shutoff value activated!", 8, offset += 10, color);
		}
		else if (fuelTicksRemaining > 0 || fuelTicksRemainingEpochs > 0) {
		...
		if (flowNetwork.isValid()) {
			...
			else if (flowNetwork.pumpShutOffValve == true)
				this.fontRendererObj.drawString("Powered pump shuts off system", 8, offset += 10, color);
			else
