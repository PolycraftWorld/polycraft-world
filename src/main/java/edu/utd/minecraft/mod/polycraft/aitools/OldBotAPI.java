...
import edu.utd.minecraft.mod.polycraft.aitools.observations.ObservationScreen;
...
//    	}
	...
	public static void reset(String args[]){
		EntityPlayerSP player = Minecraft.getMinecraft().thePlayer;
		OldBotAPI.pos.set(0, ((int)player.posX >> 4) << 4);
		OldBotAPI.pos.set(1, (int)player.posY);
		OldBotAPI.pos.set(2, ((int)player.posZ >> 4) << 4);
		BlockPos playerPos = new BlockPos(OldBotAPI.pos.get(0), OldBotAPI.pos.get(1),OldBotAPI.pos.get(2));
		int xMax = 16, yMax = 0, zMax = 16;
		
		
		if(args.length > 3) {
			OldBotAPI.pos.set(3, Integer.parseInt(args[1]));
			OldBotAPI.pos.set(4, Integer.parseInt(args[2]));
			OldBotAPI.pos.set(5, Integer.parseInt(args[3]));
			xMax = OldBotAPI.pos.get(3);
			yMax = OldBotAPI.pos.get(4);
			zMax = OldBotAPI.pos.get(5);
			...
			if(fromClient != null) {
	        	...
	        	if(availableCommands.containsKey(args[0].toUpperCase())){
		            ...
		            case JUMP:
		            	...
		            	break;
		            case MOVE_FORWARD:
		            	OldBotAPI.moveForward(args);
		            	break;
		            case MOVE_NORTH:
		            	OldBotAPI.moveNorth(args);
		            	break;
		            case MOVE_SOUTH:
		            	OldBotAPI.moveSouth(args);
		            	break;
		            case MOVE_EAST:
		            	OldBotAPI.moveEast(args);
		            	break;
		            case MOVE_WEST:
		            	OldBotAPI.moveWest(args);
		            	break;
		            ...
		            case MOVE_SOUTH_WEST:
		            	...
		            	break;
	            	case LOOK_NORTH:
		            	OldBotAPI.lookNorth(args);
		            	break;
		            case LOOK_SOUTH:
		            	OldBotAPI.lookSouth(args);
		            	break;
		            case LOOK_EAST:
		            	OldBotAPI.lookEast(args);
		            	break;
		            case LOOK_WEST:
		            	OldBotAPI.lookWest(args);
		            	break;
		            case SMOOTH_MOVE:
		            	OldBotAPI.smoothMove(args);
		            	break;
		            case SMOOTH_TURN:
		            	OldBotAPI.smoothTurn(args);
		            	break;
		            case SMOOTH_TILT:
		            	OldBotAPI.smoothTilt(args);
		            	break;
		            case TURN_RIGHT:
		            	OldBotAPI.turnRight(args);
		            	break;
		            case TURN_LEFT:
		            	OldBotAPI.turnLeft(args);
		            	break;
		            case BREAK_BLOCK:
		            	OldBotAPI.breakBlock(args);
		            	stepEndValue = false;	//action happens on server
		            	break;
		            case TELEPORT:
		            	OldBotAPI.teleport(args);
		            	break;
		            case TP_TO:
		            	OldBotAPI.tpto(args);
		            	break;
		            ...
		            case COLLECT_FROM_BLOCK:
		            	OldBotAPI.INSTANCE.collectFrom(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case CRAFT_CRAFTING_TABLE:
		            	OldBotAPI.craftCraftingTable(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case CRAFT_PLANKS:
		            	OldBotAPI.craftPlanks(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case CRAFT_AXE:
		            	OldBotAPI.craftAxe(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case CRAFT_STICKS:
		            	OldBotAPI.craftSticks(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case CRAFT_TREE_TAP:
		            	OldBotAPI.craftTreeTap(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case CRAFT_POGO_STICK:
		            	OldBotAPI.craftPogoStick(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case CRAFT:
		            case INV_CRAFT_ITEM:
		            	OldBotAPI.craft(args);
		            	stepEndValue = false;	// action happens on server
		            	break;
		            case INV_SELECT_ITEM:
		            	OldBotAPI.selectItem(args);
		            	break;
		            case USE:
		            	OldBotAPI.useItem(args);
		            	break;
		            case PLACE_BLOCK:
		            	OldBotAPI.placeBlock(args);
		            	stepEndValue = false;	// must wait for USE delay to complete
		            	break;
		            case PLACE_MACGUFFIN:
		            	OldBotAPI.placeMacGuffin(args);
		            	stepEndValue = false;	//action happens on server
		            	break;
		            case PLACE_STONE:
		            	OldBotAPI.placeStoneBlock(args);
		            	break;
		            case PLACE_CRAFTING_TABLE:
		            	OldBotAPI.placeCraftingTable(args);
		            	break;
		            case PLACE_TREE_TAP:
		            	OldBotAPI.placeTreeTap(args);
		            	break;
		            case GEN_NOVELTY:
		            	OldBotAPI.GenNovelty(args);
		            	break;
		            case RESET:
		            	OldBotAPI.reset(args);
		            	break;
		            case START:
		            	OldBotAPI.start(args);
		            	break;
		            case TREES:
		            	OldBotAPI.trees(args);
		            	break;
		            case SPEED:
		            	OldBotAPI.setMinecraftClientClockSpeed(args);
		            	break;
		            case DEFAULT:	//break fall through is intentional for default
		            	OldBotAPI.defaultAction(args);
		            default:
		...
		APIThread = new Thread("BOT API THREAD")
        {
            ...
            public void run()
            {
            	if(OldBotAPI.apiRunning.get()) {
        			...
        			while(OldBotAPI.apiRunning.get()) {
	                	try {
	                		int x = pos.get(0), y = pos.get(1), z = pos.get(2);
	                    	int xMax = pos.get(3), yMax = pos.get(4), zMax = pos.get(5);
		                        ...
		                        try {
		                        	...
		                        	if(fromClient.startsWith("START")) {
			                            ...
			                            mainThread.addScheduledTask(new Runnable()
			                            {
			                                ...
			                                public void run()
			                                {
			                                	OldBotAPI.start(fromClientFinal.split(" "));
			                                }
			                            });
