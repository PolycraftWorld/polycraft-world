...
public class GravelCannonGui  extends PolycraftInventoryGui<CannonInventory>{
	...
	public GravelCannonGui(GravelCannonInventory inventory, InventoryPlayer playerInventory, Inventory config) {
		super(inventory, playerInventory, 210, false);
		Texture = new ResourceLocation(PolycraftMod.MODID+":textures/gui/blank.png");
		this.useVelocity=config.params.getBoolean(0);
		this.useYaw=config.params.getBoolean(1);
		this.usePitch=config.params.getBoolean(2);
    ...
    public void initGui() {
        ...
        if(usePitch)
        {
	        buttonPPlus = new GuiButton(0, width / 2 -24+83, offsetFromScreenTop+ImageHeight - 115-50, 
	                20, 20, I18n.format("+", new Object[0]));
	        buttonPMinus = new GuiButton(0, width / 2 -82+83,offsetFromScreenTop+ImageHeight - 115-50, 
	                20, 20, I18n.format("-", new Object[0]));
	        ...
	        this.phi=Double.parseDouble(phiText.getText());
        }
    ...
    public void actionPerformed(GuiButton button) {
    	...
    	if(usePitch)
        {
	    	if (button == buttonPPlus)
	        {
	    		if(hasDouble(phiText.getText()))
	    		{
	    			this.phi=Double.parseDouble(phiText.getText());
	    			this.phi+=1;
	    			if(phi<0)
	    			...
	    			phiText.setText(Double.toString(this.phi));
	    		}
	        }
	    	
	    	if (button == buttonPMinus)
	        {
	    		if(hasDouble(phiText.getText()))
	    		{
	    			this.phi=Double.parseDouble(phiText.getText());
    ...
    public void drawScreen(int i, int j, float f) {
        ...
        if(usePitch)
        {
        	 fontRendererObj.drawSplitString("Pitch", 
                     offsetFromScreenLeft + 32+83, offsetFromScreenTop+70-50, 96, 0);
        	 this.phiText.drawTextBox();
        }
    ...
    public void onGuiClosed()
    {
    	...
    	if(useVelocity)
    	{
        	...
        	if(Double.parseDouble(velocityText.getText())>99)
    		{
        		velocityText.setText(Double.toString(99));
    		}
    	}
    	if(useYaw)
    	{
    		...
    		if(Double.parseDouble(thetaText.getText())>360)
    		{
    			thetaText.setText(Double.toString(360));
    		}
    	}
    	if(usePitch)
    	{
    		String p=phiText.getText();
    		if(p.length()==0)
        	{
        		test=false;
        	}
        	...
        	{
        		if(!(Character.isDigit(p.charAt(c)) || p.charAt(c)=='.'))
        		{
        			test=false;
        		}
        	}
    		if(Double.parseDouble(phiText.getText())>90)
    		{
    			phiText.setText(Double.toString(90));
    		}
    	}
       ...
       if(test)
       {
	    	...
	    	{
	    		this.phi=Double.parseDouble(phiText.getText());
		        this.inventory.phi=Double.parseDouble(phiText.getText());
	    	}
	        
	        PolycraftMod.proxy.sendMessageToServerCannon (x , y, z, velocity, theta, mass, phi);
    ...
    protected void keyTyped(char par1, int par2) throws IOException {
        ...
        if(usePitch)
        {
        	if(!( par2== Keyboard.KEY_E  &&  this.phiText.isFocused())) super.keyTyped(par1, par2);
        		this.phiText.textboxKeyTyped(par1, par2);
        }
    ...
    public void updateScreen()
    {
    	...
    	if(usePitch)
    	{
    		this.phiText.updateCursorCounter();
    	}
    ...
    protected void mouseClicked(int x, int y, int btn) throws IOException {
    	...
    	if(usePitch)
    	{
    		this.phiText.mouseClicked(x, y, btn);
    	}
    }
