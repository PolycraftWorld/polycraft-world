...
public class APICommandTeleport extends APICommandBase{
	...
	public APICommandTeleport(float cost, boolean noNav) {
		super(cost);
		this.noNav = noNav;
		side = Side.SERVER;
	}
	...
	public APICommandResult serverExecute(String[] args, EntityPlayerMP player) {
		try{			
				...
				if(args.length > 1) {
		    		...
		    		if(TutorialManager.INSTANCE.clientCurrentExperiment != -1) {	//can only get these values if there is a running experiment
		    			...
		    			if(!area.isVecInside(new Vec3(targetPos))) {
		    				return (new APICommandResult(args, APICommandResult.Result.FAIL, "Target is outside experiment zone", this.stepCost));
		    			}
					}
		    		
					if(args.length == 3) {	// if blockDistance is a parameter, set it
						if(isTargetBlock && Integer.parseInt(args[2]) == 0) {
							return (new APICommandResult(args, APICommandResult.Result.FAIL, "Cannot teleport 0 blocks away from a block", this.stepCost));
						}
					...
					if(!isTargetBlock) {	// target is entity, teleport on enttity
						...
						return (new APICommandResult(args, APICommandResult.Result.FAIL, "Block not accessible", this.stepCost));
					} else {
			...
			return new APICommandResult(args, Result.FAIL, "Invalid Input. Expected number", this.stepCost);
		}
	}
