...
public class VersionInfo {
    ...
    private void doFileExtract(File path) throws IOException
    {
    	...
    	String containedFile = getContainedFile();
        if (Strings.isNullOrEmpty(containedFile)) return;
    	System.out.println("Reading resource stream file " + containedFile);
        InputStream inputStream = getClass().getResourceAsStream("/"+containedFile);
        if (inputStream == null) {
        	System.out.println("Resource stream does not exist!");
        	throw new IOException("Cannot find file " + containedFile);
        }
